...
class TestDistCTR(TestFleetBase):
    ...
    def test_ctr_1ps_1tr_async_1thread_Tslice_Fdc_Fsm_Tsr_Fgeo_Twp_Fha_pn25(
        ...
        #loss值与预期相符
        self.check_data(train_data_list1[0], delta=3e-0, expect=self.single_cpu_data)
    ...
    def test_ctr_2ps_1tr_geo_async_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25(self):
    ...
    @run_with_compatibility
    def test_ctr_2ps_1tr_geo_async_1thread_Fslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25(self):
    ...
    def test_ctr_1ps_1tr_sync_1thread_Tslice_Fdc_Fsm_Tsr_Fgeo_Twp_Fha_pn25(
        ...
        # loss值与预期相符
        self.check_data(train_data_list1[0], delta=1e-0, expect=self.single_cpu_data)
    ...
    # Increment training test
    def test_ctr_1ps_1tr_increment_sync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25(
            self):
        """test_ctr_1ps_1tr_increment_sync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25."""
        TestFleetBase.__init__(self, pservers=1, trainers=1)
        self.run_params = {
            'sync_mode': 'sync',
            'cpu_num': 1,
        ...
        train_data_list1 = self.get_result(
            self._model_file, update_method='pserver')
        print('train_data_list1 is: {}'.format(train_data_list1))
        ...
        train_data_list2 = self.get_result(self._model_file, update_method='pserver')
        print('train_data_list2 is: {}'.format(train_data_list2))
        self.run_params = {
            'sync_mode': 'sync',
            'cpu_num': 1,
            'num_threads': 1,
            ...
            'push_nums': 25,
            "total_batch_size": 5,
            "model_dir": "./dist_model_ctr/final_pyreader"
        }
        train_data_list2[0] = train_data_list2[0] + self.get_result(self._model_file, update_method='pserver')[0]
        print('train_data_list2 is: {}'.format(train_data_list2))
        # 判断两个list输出是否为2
        assert len(train_data_list1) == 1
        assert len(train_data_list2) == 1
        #  两个train的loss值存在微小差距
        self.check_data(
            train_data_list1[0], delta=1e-5, expect=train_data_list2[0])
        # loss值与预期相符
        self.check_data(
            train_data_list1[0], delta=3e-0, expect=self.single_cpu_data_increment)


    def test_ctr_1ps_1tr_increment_hsync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25(
            self):
        """test_ctr_1ps_1tr_increment_hsync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25."""
        TestFleetBase.__init__(self, pservers=1, trainers=1)
        self.run_params = {
            'sync_mode': 'half_async',
            'cpu_num': 1,
    ...
    def test_ctr_2ps_2tr_increment_sync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25(
            self):
        """test_ctr_2ps_2tr_increment_sync_1thread_Tslice_Fdc_Fsm_Tsr_Tgeo_Twp_Fha_pn25."""
        TestFleetBase.__init__(self, pservers=2, trainers=2)
        self.run_params = {
            'sync_mode': 'sync',
            'cpu_num': 1,
        ...
        tmp = self.get_result(self._model_file, update_method='pserver')
        train_data_list2[0] = train_data_list2[0] + tmp[0]
        train_data_list2[1] = train_data_list2[1] + tmp[1]
        print('train_data_list2 is: {}'.format(train_data_list2))
        # 判断两个list输出是否为2
        assert len(train_data_list1) == 2
        assert len(train_data_list2) == 2
